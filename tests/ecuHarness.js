 ...
 */
export function makeEngine({ preset, build: patch = {}, cal: calPatch = {}, env, faults } = {}) {
  ...
  const ecu = (cond) => ({ cal, hw, cond: cond ?? S.dynoConditions(env, faults) });
  return {
 ...
 * @param {object} opts
 */
export function runLive(eng, { seconds, pedal = () => 0, coolantC = 85, aux = {}, from } = {}) {
  ...
  for (let i = 0; i < Math.round(seconds / 0.05); i++) {
    const t = i * 0.05;
    s = S.liveStep(s, 0.05, { throttle: pedal(t), load: 0 }, cfg);
